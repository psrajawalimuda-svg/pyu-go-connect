...
export default function Hotel() {
  ...
  const { data: hotels, isLoading } = useQuery({
    queryKey: ["hotels"],
    queryFn: async () => {
      // Parallel fetch hotels and rooms
      const [hotelsRes, roomsRes] = await Promise.all([
        supabase.from("hotels").select("*").eq("active", true).order("rating", { ascending: false }),
        supabase.from("hotel_rooms").select("hotel_id, price_per_night").eq("active", true),
      ]);
      ...
      return hotelsRes.data.map((h) => ({ ...h, min_price: minPrices[h.id] }));
    },
  });
